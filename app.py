...
# --- Persistência do user_id via Query Parameters ---
params = st.experimental_get_query_params()
if "user_id" not in params:
    # Gera um novo user_id se não existir na URL
    user_id = str(uuid.uuid4())
    params["user_id"] = user_id
    st.experimental_set_query_params(**params)
    st.experimental_rerun()  # Rerun para atualizar a URL com o novo user_id
else:
    user_id = params["user_id"][0]
# Armazena no session_state para facilitar o acesso interno
...
def get_supabase_client() -> Client:
    """Estabelece conexão com o Supabase utilizando variáveis de ambiente definidas nos secrets."""
...
def create_meeting_table(supabase, table_name, meeting_name, max_number=999):
    ...
    try:
        ...
        # Criar a tabela dinamicamente via RPC
        ...
        if not check_table_exists(supabase, table_name):
            raise Exception(f"Tabela {table_name} não foi criada com sucesso.")

        # Inserir números na nova tabela em lotes
        batch_size = 100
...
def generate_participant_link(table_name):
    ...
    # Inclui user_id na URL para manter o identificador do usuário
    return f"{base_url}/?table={table_name}&mode=participant&user_id={st.session_state['user_id']}"
...
if mode == "participant" and table_name_from_url:
    ...
    # Utiliza o user_id obtido via query parameters
